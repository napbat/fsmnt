...
| QNX6         | Power-Safe filesystems in either byte order; newest valid superblock snapshot selected automatically |
| BitLocker    | unlocks to the NTFS volume inside (recovery password or `.BEK`)  |

Partition tables (GPT and MBR, including extended/logical MBR partitions) and
bare unpartitioned filesystems are both handled, and filesystem type is
auto-detected from the format's identifying structures.
...
- **Journals are never replayed onto the source.** If an ext volume has a
  ...
  FAT, exFAT, APFS, Btrfs and QNX6 have no replay step. BitLocker decrypts
  in memory only.
- **A mount only succeeds when the volume is usable.** Detection refuses
  ...
  their backup boot sectors automatically; QNX6 likewise opens through its
  trailing checksummed snapshot when the primary superblock is damaged. All
  of these read through the same read-only path: the backup bytes are used
  in memory, never written back — and every such fallback is announced as a
  `warn:` line (see [Damaged and partial images](#damaged-and-partial-images)).
...
The single-volume formats (NTFS, FAT, exFAT, ext, QNX6, BitLocker) take
`default` only; any other selector is rejected at open time with
...
`fs-qnx6`, `nt-compression`, `linux-fscrypt`) are `no_std` by default with
`std` behind a feature; `nt-bitlocker` is std-only. `linux-fscrypt` is not a
parser but the kernel's file-based-encryption layer, which sits above the
filesystem drivers and is shared by all of them — `fs-ext` supplies only the
ext4 glue. Platform crates self-gate and compile to empty libraries elsewhere,
so a workspace-wide build succeeds on every OS.
...
The privately held 2017 Dodge Durango Uconnect image has an opt-in QNX6
regression that enumerates its extended MBR, opens all four Power-Safe
volumes, and reads and fingerprints every file:

```sh
FSMNT_UCONNECT_IMAGE=/path/to/uconnect.bin cargo test --test qnx6_uconnect
```
