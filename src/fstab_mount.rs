...
use crate::image_layout::ImageLayoutOptions;
use crate::{
    ImageOpenOptions, OpenedImage, OpenedPartition, PartitionOpenOptions, locate_partitions,
    open_device_partition_with_options, open_image_with_options,
};
...
impl FstabSiblings for ImageSiblings<'_> {
    ...
    fn candidates(&mut self) -> Result<Vec<Self::Address>, Box<dyn std::error::Error>> {
        ...
        let layout = crate::image_layout::image_layout_with_options(&self.path, layout_options)?;
        Ok(layout
            .partitions
            .iter()
