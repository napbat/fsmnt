...
- `crates/formats/` — parent directory (not a crate) for filesystem-format
  ...
  `fs-exfat`, `fs-qnx6`, `nt-compression`, `nt-bitlocker`, and
  `linux-fscrypt` — the format-neutral Linux file-based-encryption layer ext4
  and, in time, f2fs both call into); members via the `crates/formats/*`
  glob. Most are **vendored** from tracium; `fs-qnx6` is a first-party parser
  held to the same rules — see "Format crates" below.
- `fsmnt-drivers` — adapters binding the format parsers to
  `TargetFilesystem` / `FilesystemDriver`, so device and image mounting can
  open real partitions (NTFS, FAT12/16/32, exFAT, ext2/3/4, APFS, Btrfs,
  QNX6 Power-Safe, and BitLocker-wrapped NTFS). This is first-party code:
  full house rules apply.

## Format crates (`crates/formats/*`)

The existing tracium-derived crates are vendored; `fs-qnx6` is first-party.
Every crate here follows the same workspace lint, documentation, dependency,
layout, and 1000-line rules. Their manifests inherit `[lints] workspace =
true`, and the workspace Clippy and maximum-file-length hooks cover them
without exclusions.
...
When editing a vendored crate, prefer upstream-compatible changes where that
does not conflict with the workspace rules.
...
The `dependency-boundaries` prek/CI policy derives the workspace graph from
Cargo metadata and enforces these directions. Outside the format layer,
parsers may be consumed directly only by `fsmnt-drivers` and the fuzz harness;
the umbrella reaches them through the driver boundary.
