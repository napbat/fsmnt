//! Integration tests for leniently opening ext images that require replay.
...
fn ext_new_reports_recover_before_orphan_when_both_set() {
    let mut fs = support::load_image("ext4.img");
    support::patch_superblock_incompat(&mut fs, 0x4);
    support::patch_superblock_ro_compat(&mut fs, 0x0001_0000);
...
fn open_lenient_still_rejects_journal_dev() {
    let mut fs = support::load_image("ext4.img");
    support::patch_superblock_incompat(&mut fs, 0x8);
