...
pub fn load_image(name: &str) -> Cursor<Vec<u8>> {
...
pub fn open_ext(name: &str) -> (fs_ext::Ext, Cursor<Vec<u8>>) {
...
#[allow(dead_code, reason = "used by a subset of integration test binaries")]
pub fn patch_superblock_u16(fs: &mut Cursor<Vec<u8>>, offset: usize, value: u16) {
    let buf = fs.get_mut();
    buf[1024 + offset..1024 + offset + 2].copy_from_slice(&value.to_le_bytes());
}

#[allow(dead_code, reason = "used by a subset of integration test binaries")]
pub fn patch_superblock_u32(fs: &mut Cursor<Vec<u8>>, offset: usize, value: u32) {
    let buf = fs.get_mut();
    buf[1024 + offset..1024 + offset + 4].copy_from_slice(&value.to_le_bytes());
}

#[allow(dead_code, reason = "used by a subset of integration test binaries")]
pub fn patch_superblock_incompat(fs: &mut Cursor<Vec<u8>>, bits_to_set: u32) {
    let buf = fs.get_mut();
...
#[allow(dead_code, reason = "used by a subset of integration test binaries")]
pub fn patch_superblock_ro_compat(fs: &mut Cursor<Vec<u8>>, bits_to_set: u32) {
    let buf = fs.get_mut();
