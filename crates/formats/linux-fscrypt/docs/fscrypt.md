...
Read-only Linux fscrypt v1 / v2. Mirrors the kernel's `fs/crypto/` so a
fixture encrypted by a real Linux kernel decrypts byte-for-byte here.

fscrypt sits above the filesystem drivers in the kernel, and every
filesystem that enables it — ext4, f2fs, UBIFS, CephFS — calls the same
code. So does this crate: it takes a parsed policy, a keystore, an
object number, and the handful of filesystem facts in
[`FsParams`](../src/params.rs), and hands back a cipher. It never reads
a disk.

The filesystem-specific half is small, and today `fs-ext` is the only
host: it fetches the policy from the `encryption.c` xattr, answers the
stable-inode question from the `STABLE_INODES` compat feature, owns the
keystore, and translates this crate's `FscryptError` into its own
`ExtError`. Sections below marked **(ext4 glue)** describe that half —
a future f2fs host would supply its own equivalents while the crypto
...
- `fs/ext4/crypto.c`, `fs/ext4/namei.c` (htree dispatch — ext4 glue)
...
Anything outside this matrix surfaces as `FscryptError::UnsupportedMode`
or `FscryptError::InvalidPolicy` at access time (`fs-ext` renames these
to `ExtError::UnsupportedFscryptMode` /
`ExtError::InvalidFscryptPolicy`) -- the filesystem opens, but objects
under the unsupported policy report the failure when read.

## Key derivation

### v1 (`linux_fscrypt::kdf_v1`)
...
Policies with these flags are read using the same on-disk formats:
...
- **Stable-inode requirement**: kernel
  ...
  `EXT4_FEATURE_COMPAT_STABLE_INODES` (0x0800). The host answers the
  same question through `FsParams::has_stable_inodes`, and any
  `IV_INO_LBLK_*` policy on a filesystem that answers `false` is
  rejected fail-closed with `UnsupportedMode`. Without that guarantee
...
This crate walks each fs-block in `data_unit_size`-byte chunks during
...
`cts.rs` implements AES-256-CBC-CTS (variant CS3 -- the kernel's
...
### No-key directory entry encoding (ext4 glue)
...
`struct fscrypt_symlink_data` itself is fscrypt's, shared by every host;
what differs per filesystem is only how the raw target bytes are stored
before this framing is reached (ext4: short-inline, inline-overflow, or
block-mapped — see the note at the end of this section).
...
`decode_symlink` validates the length prefix against
the inode size, decrypts via the per-inode filenames key, and trims
trailing NUL padding to recover the original target.

**(ext4 glue)** When the registered keystore has no key for the
symlink's policy, `ExtInode::read_symlink` mirrors the kernel's
...
`content/` implements AES-256-XTS via the `xts-mode` crate.
...
`fs-ext`'s implementation in `src/hctr2.rs` is a
...
end Android phones. Adiantum is supported read-only under both v1
...
## Casefold + encryption (#123) (ext4 glue)
...
key (HKDF context 5, `DIRHASH_KEY`). This crate's `dirhash_key` /
`siphash24` implement the derivation and the primitive; fs-ext's
`htree` dispatches to them via its `dirhash_key_for_directory` glue.
Lookup decrypts entries as it goes so callers compare plaintext UTF-8
(or its NFD-folded form for casefold).
...
- `FscryptError::KeyUnwrapFailed { policy_kind, key_ref, reason }`
  (`ExtError::FscryptKeyUnwrapFailed` in fs-ext)
...
- `FscryptError::MissingKey` (`ExtError::MissingFscryptKey`) continues
  to mean "no entry registered at all under this identifier" — i.e. the
  operator never called `add_fscrypt_v2_*_key` for it.

`inode` is `0` in `KeyUnwrapFailed` because the keystore lookup
...
integrity check above this crate.
...
The kernel supports several modes and policy flags this crate does
not:
...
Encountering any of these surfaces as `UnsupportedMode` or
`InvalidPolicy`, never as silent garbage.
...
- `FscryptKeyDescriptor` and `FscryptKeyIdentifier` implement
  ...
  registered.
- All keys are supplied out-of-band; this crate does no auto-discovery
  and does not touch the kernel keyring. Operators feed master keys via
  `Ext::add_fscrypt_v1_key` / `add_fscrypt_v2_key`.

## Worked example: traverse an encrypted directory (ext4 glue)
