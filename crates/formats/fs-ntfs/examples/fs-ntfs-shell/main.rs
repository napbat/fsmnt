...
fn main() -> Result<()> {
    ...
    println!("**********************************************************************");
    println!("ntfs-shell - Demonstration of the fs-ntfs crate");
    println!("**********************************************************************");
    println!();
    ...
    T: Read + Seek,
{
    ...
    for attribute in attributes {
        ...
        if ty == NtfsAttributeType::AttributeList {
            ...
            while let Some(entry) = list_iter.try_next(&mut info.fs)? {
    ...
    T: Read + Seek,
{
    ...
    while let Some(entry) = iter.try_next(&mut info.fs)? {
        let file_name = entry
            .key()
            .expect("key must exist for a found Index Entry")?;
    ...
    T: Read + Seek,
{
    ...
    while let Some(attribute_item) = attributes.try_next(&mut info.fs)? {
