...
impl EntryValidation {
    ...
    pub fn score(&self) -> u8 {
        self.namespace_valid as u8
            + self.name_valid as u8
            + self.sizes_consistent as u8
            + self.timestamps_plausible as u8
            + self.parent_matches as u8
            + self.mft_ref_in_range as u8
    }
}
...
impl<'a, E> NtfsDirectoryEntry<'a, E>
where
...
impl<'s> NtfsSlackEntryScanner<'s> {
    ...
    fn try_parse_at(&self, offset: usize) -> Option<(NtfsRecoveredEntry, usize)> {
        ...
        let index_entry_length = u16::from_le_bytes([entry_data[8], entry_data[9]]) as usize;
        let key_length = u16::from_le_bytes([entry_data[10], entry_data[11]]) as usize;
    ...
    fn try_parse_zeroed_key(
        ...
        let name_length = entry_data[80] as usize;
        if name_length == 0 {
            return None;
        }
...
impl<'s> Iterator for NtfsSlackEntryScanner<'s> {
...
mod tests {
    ...
    fn build_file_name_key(
        ...
        let name_chars = (name_utf16.len() / 2) as u8;
    ...
    fn valid_entry_bytes(parent_record: u64, file_record: u64) -> Vec<u8> {
        ...
        let key_len = key.len() as u16; // 68
        build_index_entry(file_record, MIN_ENTRY_SIZE as u16, key_len, &key)
    }
    ...
    fn test_validate_sizes_inconsistent() {
        ...
        let bytes = build_index_entry(42, MIN_ENTRY_SIZE as u16, key.len() as u16, &key);
        let recovered = scan(&bytes, 5);
        assert_eq!(recovered.len(), 1);
        ...
        let bytes_eq = build_index_entry(42, MIN_ENTRY_SIZE as u16, key_eq.len() as u16, &key_eq);
    ...
    fn test_validate_mft_ref_in_range_boundary() {
        ...
        let at_max = build_index_entry(max, MIN_ENTRY_SIZE as u16, key.len() as u16, &key);
        let rec_at = scan(&at_max, 5);
        assert!(rec_at[0].validation().mft_ref_in_range);

        // One above the max -> out of range.
        let over = build_index_entry(max + 1, MIN_ENTRY_SIZE as u16, key.len() as u16, &key);
    ...
    fn test_try_parse_normal_rejects_unaligned_entry_length() {
        ...
        let name = [b'A', 0];
        let key = build_file_name_key(5, PLAUSIBLE_TS, 4096, 4096, 1, &name);
        let bytes = build_index_entry(42, 85, key.len() as u16, &key);
    ...
    fn test_try_parse_normal_rejects_short_key_length() {
        ...
        let name = [b'A', 0];
        let key = build_file_name_key(5, PLAUSIBLE_TS, 4096, 4096, 1, &name);
        let mut data = build_index_entry(42, MIN_ENTRY_SIZE as u16, 67, &key);
        data.resize(200, 0);
        let recovered = scan(&data, 5);
        assert!(
            recovered
    ...
    fn valid_entry_len88(parent_record: u64, file_record: u64) -> Vec<u8> {
        ...
        let key = build_file_name_key(parent_record, PLAUSIBLE_TS, 4096, 4096, 1, &name);
        let key_len = key.len() as u16; // 70
        let entry_len = round_up_4(INDEX_ENTRY_HEADER_SIZE + key_len as usize) as u16; // 88
        let mut entry = build_index_entry(file_record, entry_len, key_len, &key);
        entry.resize(entry_len as usize, 0);
        entry
    }
    ...
    fn test_normal_path_rejects_entry_length_exceeding_data() {
        ...
        let key = build_file_name_key(5, PLAUSIBLE_TS, 4096, 4096, 1, &name);
        let mut bytes = build_index_entry(42, 88, key.len() as u16, &key);
