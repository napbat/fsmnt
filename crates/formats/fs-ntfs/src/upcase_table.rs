...
mod tests {
    ...
    fn ascii_upcase_table() -> UpcaseTable {
        let mut chars: Vec<u16> = (0u32..UPCASE_CHARACTER_COUNT as u32)
            .map(|c| c as u16)
            .collect();
        for c in b'a'..=b'z' {
            chars[c as usize] = (c - 32) as u16;
        }
        UpcaseTable {
    ...
    fn upcase_table_size_constant() {
        ...
        assert_eq!(UPCASE_TABLE_SIZE, 131072);
    }
    ...
    fn u16_to_uppercase_maps_per_table() {
        ...
        assert_eq!(table.u16_to_uppercase(b'a' as u16), b'A' as u16);
        assert_eq!(table.u16_to_uppercase(b'z' as u16), b'Z' as u16);
