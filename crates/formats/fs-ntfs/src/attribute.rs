...
fn validated_bytes<const N: usize>(data: &[u8], start: usize) -> [u8; N] {
    data.get(start..)
        .and_then(|bytes| bytes.first_chunk())
        .copied()
        .expect("attribute construction validates every fixed-width header field")
}

fn validated_usize(value: u32) -> usize {
    usize::try_from(value).expect("u32 NTFS structure sizes fit usize on supported targets")
}
...
impl<'n, 'f> NtfsAttribute<'n, 'f> {
    ...
    pub fn attribute_length(&self) -> u32 {
        ...
        u32::from_le_bytes(validated_bytes(self.file.record_data(), start))
    }
    ...
    pub fn flags(&self) -> NtfsAttributeFlags {
        ...
        NtfsAttributeFlags::from_bits_truncate(u16::from_le_bytes(validated_bytes(
            self.file.record_data(),
            start,
        )))
    }
    ...
    pub fn instance(&self) -> u16 {
        ...
        u16::from_le_bytes(validated_bytes(self.file.record_data(), start))
    }
    ...
    pub fn name(&self) -> Result<U16StrLe<'f>> {
        ...
        let start = self.offset + usize::from(self.name_offset());
    ...
    pub fn name_length(&self) -> usize {
        ...
        usize::from(name_length_in_characters) * mem::size_of::<u16>()
    }
    ...
    pub(crate) fn non_resident_value_data_and_position(&self) -> Result<(&'f [u8], NtfsPosition)> {
        ...
        let start = self.offset + usize::from(self.non_resident_value_data_runs_offset());
        let end = self.offset + validated_usize(self.attribute_length());
    ...
    fn non_resident_value_data_size(&self) -> u64 {
        ...
        u64::from_le_bytes(validated_bytes(self.file.record_data(), start))
    }
    ...
    pub(crate) fn resident_value(&self) -> Result<NtfsResidentAttributeValue<'f>> {
        ...
        let start = self.offset + usize::from(self.resident_value_offset());
        let end = start + validated_usize(self.resident_value_length());
    ...
    pub fn ty(&self) -> Result<NtfsAttributeType> {
        ...
        let ty = u32::from_le_bytes(validated_bytes(self.file.record_data(), start));
    ...
    fn validate_attribute_length(&self) -> Result<()> {
        ...
        let attribute_length = validated_usize(self.attribute_length());
    ...
    fn validate_name_sizes(&self) -> Result<()> {
        ...
        if u32::from(start) >= self.attribute_length() {
        ...
        let end = usize::from(start) + self.name_length();
        if end > validated_usize(self.attribute_length()) {
    ...
    fn validate_resident_value_sizes(&self) -> Result<()> {
        ...
        if u32::from(start) > attribute_length {
    ...
    pub fn value_length(&self) -> u64 {
        if self.is_resident() {
            u64::from(self.resident_value_length())
        } else {
...
impl<'n, 'f, T> Iterator for NtfsAttributesAttached<'n, 'f, '_, T>
where
    T: Read + Seek,
{
...
impl<T> FusedIterator for NtfsAttributesAttached<'_, '_, '_, T> where T: Read + Seek {}
...
impl<'n, 'f> NtfsAttributesRaw<'n, 'f> {
    pub(crate) fn new(file: &'f NtfsFile<'n>) -> Self {
        let start = usize::from(file.first_attribute_offset());
        let end = validated_usize(file.data_size());
...
impl<'n, 'f> Iterator for NtfsAttributesRaw<'n, 'f> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let ty = u32::from_le_bytes(validated_bytes(ty_slice, 0));
        if ty == NtfsAttributeType::End.as_u32() {
            return None;
        }

        // It's a real attribute.
        let attribute = iter_try!(NtfsAttribute::new(self.file, self.items_range.start, None));
        self.items_range.start += validated_usize(attribute.attribute_length());
