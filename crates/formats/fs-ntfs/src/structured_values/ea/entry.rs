...
impl<'s> NtfsEaEntry<'s> {
    fn new(data: &'s [u8], position: NtfsPosition) -> Result<Self> {
        ...
        let value_len = header.ea_value_length.get() as usize;
    ...
    pub fn value(&self) -> &[u8] {
        let name_len = self.name_length() as usize;
...
impl<'s> Iterator for NtfsEaEntries<'s> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let header = match EaEntryHeader::read_from_bytes(&remaining[..EA_ENTRY_HEADER_SIZE]) {
            Ok(h) => h,
            Err(_) => {
                self.finished = true;
                return None;
        ...
        let next_offset = header.next_entry_offset.get() as usize;
        ...
        let entry_slice = if next_offset == 0 {
            ...
            if next_offset > remaining.len() {
                ...
                return Some(Err(NtfsError::InvalidStructuredValueSize {
                    ...
                    ty: NtfsAttributeType::EA,
                    expected: (self.offset + next_offset) as u64,
...
impl<'n, 'f> NtfsStructuredValueFromResidentAttributeValue<'n, 'f> for NtfsEa {
...
impl<'a> arbitrary::Arbitrary<'a> for NtfsEa {
    fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
        ...
        let name_bytes = u.bytes(name_len as usize)?;
        data.extend_from_slice(name_bytes);
        data.push(0); // null terminator
        let value_bytes = u.bytes(value_len as usize)?;
...
mod tests {
    ...
    fn test_ea_single_entry() {
        ...
        let ea = NtfsEa::new(&mut cursor, NtfsPosition::none(), data.len() as u64)
    ...
    fn test_ea_next_offset_beyond_remaining_errors() {
        ...
        let mut entry1 = build_ea_entry(b"A", b"1", 0x00, 0);
        let first_len = entry1.len() as u32;
        ...
        let bogus_next = (entry2.len() + 100) as u32;
        entry2[..4].copy_from_slice(&bogus_next.to_le_bytes());

        let mut data = entry1;
        let offset_at_entry2 = data.len() as u32;
        data.extend_from_slice(&entry2);

        let mut cursor = ReadOnlyCursor::new(&data);
        let ea = NtfsEa::new(&mut cursor, NtfsPosition::new(0x100), data.len() as u64)
            .expect("should parse");
